...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      ...
      {/* Logo and Text */}
      <div
        className={`flex items-center w-full px-6 pr-14 lg:pr-6 transition-all duration-500 ease-out ${isOpen ? "mb-8" : "mb-6"
          }`}
      >
        <div
          className={`rounded-full bg-white p-1 shrink-0 transition-all duration-300 ease-in-out ${isOpen ? "w-14 h-14 lg:w-20 lg:h-20" : "w-12 h-12"
        ...
        >
          <img
            ...
            className={`w-auto object-contain transition-all duration-300 ease-in-out ${isOpen ? "h-20" : "h-16"
              }`}
          />
        </div>
      </div>
...
const SidebarItem = ({ src, label, isOpen, isActive, onClick }) => (
  <div className="w-full relative" onClick={onClick}>
    ...
    <div
      className={`flex items-center cursor-pointer transition-all duration-300 px-4 ${isOpen ? "py-3" : "py-2"
        } mx-4`}
    >
      {/* Icon with conditional white container */}
      <div className="relative shrink-0 mr-3">
        <div
          className={`rounded-lg p-2 transition-all ${isActive ? "bg-white shadow-md" : "bg-transparent hover:bg-white/10"
            }`}
        >
          <img
            src={src}
