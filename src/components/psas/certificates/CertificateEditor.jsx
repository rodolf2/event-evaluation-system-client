...
const CertificateEditor = ({ onBack, initialData }) => {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-1">
            <button disabled={!isText} onClick={() => updateProperty("textAlign", "left")} className={`p-1 md:p-2 border rounded-md disabled:bg-gray-100 disabled:cursor-not-allowed disabled:text-gray-400 ${getProp("textAlign", "left") === "left" ? "bg-gray-200" : ""}`}><AlignLeft size={16} /></button>
            <button disabled={!isText} onClick={() => updateProperty("textAlign", "center")} className={`p-1 md:p-2 border rounded-md disabled:bg-gray-100 disabled:cursor-not-allowed disabled:text-gray-400 ${getProp("textAlign", "center") === "center" ? "bg-gray-200" : ""}`}><AlignCenter size={16} /></button>
            <button disabled={!isText} onClick={() => updateProperty("textAlign", "right")} className={`p-1 md:p-2 border rounded-md disabled:bg-gray-100 disabled:cursor-not-allowed disabled:text-gray-400 ${getProp("textAlign", "right") === "right" ? "bg-gray-200" : ""}`}><AlignRight size={16} /></button>
          </div>
        </div>
  ...
  return (
    <div className="bg-gray-100 p-4 font-sans h-full min-h-screen">
      <div className="flex flex-col lg:flex-row h-full bg-white rounded-lg shadow-md overflow-hidden">
        <div className="w-full lg:w-72 p-4 border-b lg:border-b-0 lg:border-r overflow-y-auto flex flex-col gap-6">
          <div>
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-col gap-2">
                <button onClick={() => fileInputRef.current.click()} className="flex items-center justify-center gap-2 p-2 border rounded-md hover:bg-gray-100"><Upload size={16} /> Upload</button>
                <input type="file" ref={fileInputRef} onChange={handleImageUpload} className="hidden" accept="image/*" />
                <button onClick={addImageFromUrl} className="flex items-center justify-center gap-2 p-2 border rounded-md hover:bg-gray-100 text-sm">From URL</button>
                <button onClick={() => bgInputRef.current.click()} className="flex items-center justify-center gap-2 p-2 border rounded-md hover:bg-gray-100">Set Background</button>
                <input type="file" ref={bgInputRef} onChange={handleBackgroundUpload} accept="image/*" className="hidden" />
                <button onClick={() => { fabricCanvas.current?.setBackgroundImage(null, fabricCanvas.current.renderAll.bind(fabricCanvas.current)); }} className="flex items-center justify-center gap-2 p-2 border rounded-md hover:bg-gray-100 text-sm">Clear BG</button>
              </div>
            </div>
            <div>
        ...
        <div className="grow flex justify-center items-center bg-gray-200 p-4 lg:p-8" ref={canvasContainerRef}>
          <div className="bg-white shadow-lg">
            <canvas ref={canvasRef} />
          </div>
        </div>

        <div className="w-full lg:w-80 p-4 border-t lg:border-t-0 lg:border-l overflow-y-auto relative bg-white">
          <PropertiesPanel />
