...
// Custom minimal sidebar for guest evaluators - always shows Evaluations as active
const GuestEvaluatorSidebar = ({ isOpen, onClose }) => {
  return (
    <aside
    ...
    >
      ...
      {/* Navigation - Single Evaluations item, always active, no navigation */}
      <nav className="flex flex-col space-y-2 w-full">
        <div className="w-full relative">
          ...
          <div className="flex items-center gap-3 px-4 py-3 bg-white text-[#1F3463] rounded-lg mx-4 cursor-default">
            ...
            {isOpen && (
              <span className="text-sm font-medium text-[#1F3463]">
                Evaluations
              </span>
            )}
          </div>
...
// Custom header for guest evaluators (no auth required)
const GuestEvaluatorHeader = ({ onMenuClick }) => {
  return (
    <header className="sticky top-0 flex items-center justify-between bg-white shadow-sm p-4 rounded-lg z-20 hover:shadow-lg print:hidden">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="font-semibold text-gray-700 text-lg">Evaluations</h1>
      </div>
...
function GuestEvaluatorLayout({ children }) {
  ...
  return (
    ...
    >
      <GuestEvaluatorSidebar isOpen={isSidebarOpen} onClose={toggleSidebar} />
      ...
      >
        <GuestEvaluatorHeader onMenuClick={toggleSidebar} />
        <div>{children}</div>
      </main>
    </div>
  );
...
export default GuestEvaluatorLayout;
